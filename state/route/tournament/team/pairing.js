function matchRoute(db, redis, socketWrapper) {
  function pairingList(global) {
    db.query(
      `SELECT garden_player.team_id AS playerTeamId,
              garden_player.name,
              garden_player.handle,
              garden_player.rating,
       ...
       FROM garden_player
                LEFT JOIN garden_pairing home_pairing ON (garden_player.team_id = home_pairing.home_id)
                LEFT JOIN garden_team home_team ON (home_pairing.home_id = home_team.id)
                LEFT JOIN garden_pairing away_pairing ON (garden_player.team_id = away_pairing.away_id)
                LEFT JOIN garden_team away_team ON (away_pairing.away_id = away_team.id)
       ORDER BY garden_player.id;`,
      (err, playerList) => {
        if (err) {
  ...
  function watchPairing(id) {
    db.query(
      `SELECT pairing.id,
              garden_player.team_id AS playerTeamId,
              garden_player.name,
              garden_player.handle,
              garden_player.rating,
              home_pairing.id      AS homeTeamPairingId,
       ...
       FROM garden_player
                ...
                LEFT JOIN garden_pairing away_pairing ON (garden_player.team_id = away_pairing.away_id)
                LEFT JOIN garden_team away_team ON (away_pairing.away_id = away_team.id)
                INNER JOIN garden_pairing pairing ON (pairing.id = ? AND (garden_player.team_id = pairing.home_id OR
                                                                         garden_player.team_id = pairing.away_id))
       ORDER BY garden_player.id;`,
      [id],
      (err, playerList) => {
        if (err) {
        ...
        redis.hset('garden:stream:state', hset, (err) => {
          if (err) {
          ...
          matchUpDate.forEach((item) => redis.set(`garden:stream:board:${item[0]}`, item[1].toLowerCase()));
          redis.hset('garden:stream:board', hset, (err) => {
            if (err) {
